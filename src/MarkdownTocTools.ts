...
export class MarkdownTocTools {
    ...
    private insertAnchor(editBuilder: TextEditorEdit, headerList: any[]) {
        if (!this.configManager.options.INSERT_ANCHOR)
            return;
        headerList.forEach(element => {
    ...
    private createToc(editBuilder: TextEditorEdit, headerList: any[], insertPosition: Position) {
        ...
        let indicesOfDepth = Array.apply(null, new Array(this.configManager.options.DEPTH_TO - this.configManager.options.DEPTH_FROM + 1)).map(Number.prototype.valueOf, 0);
        ...
        let startDepth = Math.max(minDepth, this.configManager.options.DEPTH_FROM);
        headerList.forEach(element => {
            if (element.depth <= this.configManager.options.DEPTH_TO) {
                ...
                let row = [
                    tab.repeat(length),
                    this.configManager.options.ORDERED_LIST ? (++indicesOfDepth[length] + '. ') : '- ',
                    this.configManager.options.WITH_LINKS ? element.hash : element.title
                ];
                text.push(row.join(''));
    ...
    private getHeaderList() {
        ...
        if (editor != undefined) {
            ...
            for (let index = 0; index < doc.lineCount; index++) {
                ...
                if (depth < this.configManager.options.DEPTH_FROM)
                    continue;
                if (depth > this.configManager.options.DEPTH_TO)
                    continue;
                if (lineText.match(this.configManager.optionKeys.REGEXP_IGNORE_TITLE))
                    continue;
                for (var i = depth; i <= this.configManager.options.DEPTH_TO; i++) {
                    indicesOfDepth[depth] = 0;
                }
                indicesOfDepth[depth - 1]++;
                let orderedListStr = "";
                for (var i = this.configManager.options.DEPTH_FROM - 1; i < depth; i++) {
                ...
                let hash = this.getHash(title, this.configManager.options.ANCHOR_MODE, hashMap[title]);
